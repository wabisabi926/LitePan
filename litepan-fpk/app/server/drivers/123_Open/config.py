...
@dataclass
class Pan123OpenConfig(DriverConfig):
    access_token: str = ""
    refresh_token: str = ""
    root_folder_id: str = "0"
    delete_mode: str = "trash"      # 123 Open 当前只支持删除到回收站
    ...
    def __post_init__(self):
        ...
        if self.delete_mode != "trash":
            raise ValueError("123云盘Open当前只支持删除到回收站")
    ...
    def get_cache_config(self) -> Dict[str, Any]:
        token_key = self.access_token or self.refresh_token
        return {
            "ttl": self.cache_ttl * 60 if self.cache_ttl is not None else None,
            "key_prefix": f"123_open_{hash(token_key)}",
        }
    ...
    def get_form_schema(cls) -> Dict[str, Any]:
        return {
            "access_token": {
                ...
                "required": True,
                "label": "访问令牌",
                "description": "123云盘开放API access_token",
            ...
            "refresh_token": {
                ...
                "required": True,
                "label": "刷新令牌",
                "description": "123云盘开放API refresh_token，用于自动续期访问令牌",
            ...
            "delete_mode": {
                ...
                "required": False,
                "label": "删除模式",
                "description": "123云盘Open当前仅支持删除到回收站，后续如官方开放彻底删除接口再扩展",
                "default": "trash",
                "options": [
                    {"value": "trash", "label": "删除到回收站"},
                ],
            },
            "download_mode": {
                "type": "select",
